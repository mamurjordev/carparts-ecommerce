<template>
            ...
            <div class="container">
                <div class="mobile-header__body">
                    ...
                    <!-- <a class="mobile-header__logo" href="">
                        <img src="/media/site/logo.png" alt="" />
                    </a> -->
                    <div
                        class="mobile-header__search mobile-search position-relative"
                    >
                        <form class="mobile-search__body">
                            <input
                                ...
                                v-model="search"
                                placeholder="Suche nach Teile- oder OEM-Nummer"
                            />
                            <button
                                type="button"
 ...
 C15.5,10.2,14,11.3,14,12z"
                                ...
                                <span
                                    class="mobile-search__vehicle-picker-label"
                                    >Vehicle</span
                                >
                            </button>
                            <button
 ...
 C17.1,15.7,17.1,16.3,16.7,16.7z"
                        ...
                        <div
                            v-if="search"
                            class="position-absolute search_result"
                        >
                            <div v-if="progress.search" class="text-center">
                                <div
                            ...
                            <div v-else class="">
                                <div v-if="searches" class="flex flex-col">
                                    <inertia-link
                                        v-for="(search,
                                        index) in searches.parts"
                                        :key="index"
                                    ...
                                    >
                                        <div class="suggestions__product-info">
                                            <div
                                                class="suggestions__product-name"
                                            >
                                                {{ search.assemblyGroupName }}
                                            </div>
                                        </div>
                                    </inertia-link>
                                    <inertia-link
                                        v-for="(search, index) in searches.oems"
                                        :key="index + 100"
                                        class="suggestions__item suggestions__product"
                                        href="/products"
                                    >
                                ...
                                <div v-else class="">Keine Teile gefunden</div>
                            </div>
                        </div>
                    </div>
                    <div class="mobile-header__indicators">
                        <div
                            class="mobile-indicator mobile-indicator--search d-md-none"
                        >
                            <button
                                type="button"
...
export default {
    ...
    methods: {
        getSearch(search) {
            console.log("ok");
            this.progress.search = true;
            this.$http
                .get("/api/v1/search/" + search)
                .then(
                    response => (
                        (this.searches = response.data),
                        (this.progress.search = false)
                    )
                );
        },
        closeSearch() {
            (this.search = ""),
                (this.searches.oems = null),
                (this.searches.parts = null);
        }
    }
};
</script>
